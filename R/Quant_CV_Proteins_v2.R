...
fwrite(SCH[Booster %like% "No|HEY" & AGC == "AGC300%" & Amount == "50pg" & Method == "SN_Corrected" & Rep == "2" & is.na(CV) == FALSE],"tables/Fig1c.txt")

pdf("figures/Proteins_ID&CV.pdf",width = 5,height = 5)
...
SCH[Booster %like% "No|HEY" & AGC == "AGC300%" & Amount == "50pg" & Method == "Int_Corrected" & Rep == "2" & is.na(CV) == FALSE]%>%
  ggplot(aes(x = Ratio, fill = CV <= 20)) + 
  geom_bar(position = position_stack(),alpha = 0.9) + 
  geom_text(stat='count', aes(label=..count..), position = position_stack(),
            vjust=0.2, size = 5)+ scale_fill_brewer(palette = "Set2") + theme_prism()
...
pdf("figures/Proteins_ID&CV_AllMethods.pdf",width = 10,height = 10)

SCH[Booster %like% "No|HEY" & AGC == "AGC300%" & Amount == "50pg" & Rep == "2" & is.na(CV) == FALSE]%>%
  ggplot(aes(x = Ratio, fill = CV <= 20)) + 
  geom_bar(position = position_stack(),alpha = 0.9) + 
  geom_text(stat='count', aes(label=..count..), position = position_stack(),
            vjust=0.2, size = 5)+ scale_fill_brewer(palette = "Set2") +facet_wrap(Method~.)+ theme_prism()
...
ggplot(SCH[Av14SN >= 1 & Booster %like% "HEY|No" & Method == "SN_Corrected" & Amount == "50pg"  & Rep == "2"], 
...
fwrite(SCH[Av14SN >= 1 & Booster %like% "HEY|No" & Method == "SN_Corrected" & Amount == "50pg"  & Rep == "2"],"tables/Fig.S2bc")
