...
SC[,Av12SN := sum(c(SN128N,SN129N,SN129C,SN130N,SN130C,
...
# ggplot(SC,aes(x = log10(Av12SN), y = log10(RawOvFtT), color = NCE)) + geom_point()
...
SC2 <- SC[,.(Raw,`First Scan`,Precursor,Organism,Av12SN,SN126,RawOvFtT,XCorr)]%>%unique()

SC2[,rank := rank(-Av12SN),by=.(Raw,Precursor)]
...
SC3 <- dcast(SC3,Precursor + ID + Ratio ~ NCE, value.var = "Av12SN")
...
SC4 <- dcast(SC4,Precursor + ID + Ratio ~ Resolution, value.var = "Av12SN")
...
SC2.2[N == 48 & Resolution == "60K" & Av12SN >= 1 & ID %% 20 == 4]%>%
  ggplot(aes(x=NCE, y=log10(Av12SN),group = Precursor,color = NCE)) +
...
SC2[Resolution == "60K" & Av12SN >= 1 & ID %% 10 == 1]%>%
  ggplot(aes(x=NCE, y=log10(Av12SN),group = Precursor,color = NCE)) +
  geom_line(color = "grey") +
  geom_point() + facet_grid(. ~ Ratio) + theme_prism(base_size = 10) 
...
SC2[NCE == "32%" & Av12SN >= 1 & ID %% 10 == 1]%>%
  ggplot(aes(x=Resolution, y=log10(Av12SN),group = Precursor,color = Resolution)) +
  geom_line(color = "grey") +
  geom_point() + facet_grid(. ~ Ratio) + theme_prism(base_size = 10) 
...
SC2[Resolution == "60K" & Ratio == "42" & Av12SN >= 1 & ID %% 5 == 1]%>%
  ggplot(aes(x=NCE, y=log10(Av12SN),group = Precursor,color = NCE)) +
  geom_line(color = "grey") +
...
SC[,mean := mean(c(SN128N,SN129N,SN129C,SN130N,SN130C,SN131N,SN131C,
...
ggplot(SC[Av12SN >= 1 & Resolution == "60K"], 
