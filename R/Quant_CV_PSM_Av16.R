...
All[,Av16SN := sum(c(SN126,SN127N,SN127C,SN128N,SN128C,SN129N,SN129C,SN130N,
                     SN130C,SN131N,SN131C,SN132N,SN132C,SN133N,SN133C,SN134N),
                  na.rm = TRUE)/16, by = ID]
...
pdf("figures/CV_PSMs_Av16.pdf",width = 10,height = 5)

ggplot(SCH[Av16SN >= 1 & Booster == "HEY" & Amount == "50pg" & is.na(CV) == FALSE], 
...
ggplot(SCH[Av16SN >= 1 & Booster %like% "No|HEY" & AGC == "AGC300%" & Amount == "50pg"], 
       aes(x = log2(Av16SN), y = log2(CV))) + 
...
ggplot(SCH[Av16SN >= 1 & Booster %like% "No|HEY" & Method == "SN_Corrected" & Amount == "50pg"], 
       aes(x = log2(Av16SN), y = log2(CV))) + 
  geom_point(size = 0.3, color = "skyblue")+
...
ggplot(SCH[Av16SN >= 1 & Booster %like% "No|HEY" & Method == "SN_Raw" & Amount == "50pg"], 
       aes(x = log2(Av16SN), y = log2(CV))) + 
  geom_point(size = 0.3, color = "skyblue")+
  facet_grid(AGC~Ratio) + 
  geom_hline(yintercept = log2(20), size = 1, linetype = "dashed") + theme_prism() + 
...
ggplot(SCH[Av16SN >= 1 & Booster %like% "No|HEY" & Method == "Int_Corrected" & Amount == "50pg"], 
       aes(x = log2(Av16SN), y = log2(CV))) + 
  geom_point(size = 0.3, color = "skyblue")+
  facet_grid(AGC~Ratio) + 
  geom_hline(yintercept = log2(20), size = 1, linetype = "dashed") + theme_prism() + 
...
ggplot(SCH[Av16SN >= 1 & Booster %like% "No|HEY" & Method == "Int_Raw" & Amount == "50pg"], 
       aes(x = log2(Av16SN), y = log2(CV))) + 
  geom_point(size = 0.3, color = "skyblue")+
  facet_grid(AGC~Ratio) + 
  geom_hline(yintercept = log2(20), size = 1, linetype = "dashed") + theme_prism() + 
...
ggplot(SCH[Av16SN >= 1 & Booster %like% "No|HEY" & Method == "SN_Corrected" & Amount == "50pg"], 
       aes(x = log2(Av16SN), y = log2(CV), color = log10(RawOvFtT))) + 
